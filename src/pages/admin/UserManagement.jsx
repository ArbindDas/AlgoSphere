...
import { 
  Mail, Phone, Calendar, CheckCircle, XCircle, User, 
  Edit, Trash2, UserPlus, Search, Filter, RefreshCw,
  Send, Power, MoreVertical, X, Shield, Key
} from 'lucide-react';
...
const UserManagement = () => {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (!event.target.closest('.action-menu-button') && !event.target.closest('.action-menu-dropdown')) {
  ...
  const handleFormSubmit = async (e, isEdit = false) => {
    ...
    try {
      if (isEdit && selectedUser) {
        ...
        // For new users, you might want to add password field
        const userData = {
          ...formData,
          // You might want to add password generation or require it in the form
          credentials: [{
            type: "password",
            value: "TempPassword123!", // You should generate this or add a password field
            temporary: true
          }]
        };
      ...
      setFormData({ username: '', email: '', firstName: '', lastName: '', enabled: true, emailVerified: false });
      fetchUsers();
    } catch (err) {
      alert(`Failed to ${isEdit ? 'update' : 'create'} user: ${err.message || 'Unknown error'}`);
    }
  };
  ...
  const handleResetPassword = async () => {
    ...
    try {
      await userService.resetPassword(selectedUser.id, passwordData.newPassword, passwordData.temporary);
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-purple-50 relative overflow-hidden p-8">
        ...
        <div className="relative bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl p-8 border border-white/20 max-w-md mx-auto mt-20">
          <div className="text-center">
            ...
            <p className="text-xl font-bold text-red-900 mb-2">Error loading users</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredUsers.map((user) => (
            <div
              ...
              onMouseEnter={(e) => {
                e.currentTarget.style.boxShadow = '0 20px 60px rgba(99, 102, 241, 0.4), 0 0 40px rgba(139, 92, 246, 0.3)';
              }}
              onMouseLeave={(e) => {
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex gap-1 mt-1">
                      ...
                      <span className={`px-2 py-0.5 text-xs rounded-full font-semibold ${
                        user.emailVerified ? 'bg-emerald-500 text-white' : 'bg-yellow-500 text-white'
                      }`}>
                        {user.emailVerified ? '✓' : '⚠'}
                      </span>
                    </div>
                ...
                <div className="relative z-10">
                  ...
                  <div className="flex items-center gap-2 text-gray-700">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm font-medium">{formatDate(user.createdTimestamp)}</p>
                    </div>
                  </div>
                  ...
                  {user.requiredActions && user.requiredActions.length > 0 && (
                    <div className="p-2 bg-red-50 border border-red-200 rounded-lg">
                      <p className="text-xs text-red-800 font-semibold">⚠️ Actions Required</p>
                      <p className="text-xs text-red-600 mt-0.5">{user.requiredActions.join(', ')}</p>
                    </div>
                  )}
                </div>
              </div>
          ...
          <div className="space-y-4">
            <div>
              <p className="text-sm text-gray-600 mb-4">
                Reset password for <span className="font-semibold">{selectedUser.username || selectedUser.email}</span>
              </p>
              <div>
              ...
              <div className="mt-4">
                <label className="flex items-center">
                  <input
                    ...
                    className="mr-2"
                  />
                  <span className="text-sm text-gray-700">Temporary password (requires change on next login)</span>
                </label>
              </div>
            </div>
            <div className="flex gap-3 pt-4">
